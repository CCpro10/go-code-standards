...
2. 编写或审查代码时应用 `references/go-style-rules.md` 中的规则。本地既有约定优先，除非它们与正确性、可维护性或更强的项目规范冲突。
...
- 包级声明顺序：每个 package 中，可导出的 package-level 函数和方法必须位于不可导出的函数和方法之前；private section 后禁止出现大写开头的函数或方法。
...
使用 `references/go-style-rules.md` 查看紧凑规则清单和来源链接。
